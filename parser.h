...
private:
    std::shared_ptr<ASTNode> parseFunction() {
        ...
        if (current.kind != "T_INT" && current.kind != "T_FLOAT" && current.kind != "T_BOOL" && current.kind != "T_STRING")
    ...
    std::shared_ptr<ASTNode> parseParams() {
        ...
        while (current.kind != "T_PARENR") {
            ...
            params->addChild(std::make_shared<ASTNode>("Param", type + " " + name));
    ...
    std::shared_ptr<ASTNode> parseVarDecl() {
        ...
        auto declNode = std::make_shared<ASTNode>("VarDecl");
    ...
    std::shared_ptr<ASTNode> parseExprTail(std::shared_ptr<ASTNode> left) {
        while (current.kind == "T_PLUS" || current.kind == "T_MINUS" || current.kind == "T_MUL" || current.kind == "T_DIV" ||
               current.kind == "T_EQUALSOP" || current.kind == "T_NOTEQOP" || current.kind == "T_LESSOP" || current.kind == "T_GREATOP" ||
               current.kind == "T_LEQOP" || current.kind == "T_GEQOP" || current.kind == "T_AND" || current.kind == "T_OR") 
        {
            std::string op = current.val;
            next();
